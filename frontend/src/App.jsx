...
const menuGroups = [
  {
    title: "",
    items: [
      { name: "Overview", icon: FiGrid, active: true },
    ],
  },
  {
    title: "Security",
    items: [
      { name: "Threat Dashboard", icon: FiActivity },
      { name: "URL Scanner", icon: FiLink },
      { name: "Phishing Scanner", icon: FiMessageSquare },
      { name: "Network Monitor", icon: FiGlobe },
      { name: "Behaviour Analytics", icon: FiUsers },
    ],
  },
  {
    title: "Intelligence",
    items: [
      { name: "Threat Intelligence", icon: FiTarget },
      { name: "Threat Graph", icon: FiLayers },
      { name: "Security Events", icon: FiDatabase },
    ],
  },
  {
    title: "Response",
    items: [
      ...
      { name: "Incident Center", icon: FiAlertTriangle },
    ],
  },
  {
    title: "AI",
    items: [
      { name: "AI Security Assistant", icon: FiCpu },
      { name: "Model Analytics", icon: FiBarChart2 },
    ],
  },
  {
    title: "System",
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      key={item.name}
                      className={`
                        flex w-full items-center rounded-xl border px-3 py-2.5 text-sm transition
      ...
      >
        ...
        <main className="p-4 md:p-7">
          ...
          <section className="mb-7 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Monitor AI detection modules, security events and system health.
              </p>
            </div>
              ...
              <div className="mt-5 flex min-h-[330px] items-center justify-center rounded-xl border border-dashed border-slate-800 bg-slate-950/40">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-2 max-w-sm text-sm text-slate-500">
                    Real detection data will appear after our AI modules are trained and connected.
                  </p>
            ...
            <Panel title="System Health">
              ...
              {backendData && (
                <p className="mt-4 text-xs text-slate-600">
                  Backend v{backendData.version} • {backendData.environment}
                </p>
              )}
          ...
          <footer className="mt-8 border-t border-slate-900 py-5 text-center text-xs text-slate-700">
            DeepShield AI • AI-Powered Cybersecurity Platform
          </footer>
...
function Module({ name, status, ready = false }) {
  return (
      ...
      <div className="flex items-center gap-3">
        ...
        <span
          className={`h-2 w-2 rounded-full ${
            ready ? "bg-emerald-400" : "bg-slate-700"
          }`}
        />
...
function HealthRow({ icon: Icon, name, status, active = false }) {
  return (
